...
class PydanticToHFDatasets(abc.ABC):
    ...
    def _annotation_to_values(cls, annotation: Any) -> datasets.Value | datasets.Sequence | datasets.Features:
        ...
        Handles:
          ...
          - Union/Optional => handles Optional[T], fallback to string for others
          - Catch-all fallback => string

        :param annotation: Annotation to convert.
        :return: Hugging Face dataset feature instance generated from specified annotation.
        """
        ...
        if origin in (typing.Union, getattr(types, "UnionType", None)):
            # Filter out NoneType to check for Optional
            non_none_args = [arg for arg in args if arg is not type(None)]
            if len(non_none_args) == 1:
                # It's Optional[T] or Union[T, None]
                return cls._annotation_to_values(non_none_args[0])
            # For other Unions, fallback to string
            return datasets.Value("string")

        # 5) Basic primitives. Fallback: store as string.
        primitives_map: dict[type, str] = {
            str: "string",
            int: "int32",
            float: "float32",
            bool: "bool",
    ...
    def model_to_dict(cls, model: pydantic.BaseModel | None) -> Any:
        ...
        Matches the Hugging Face Features schema defined by `model_cls_to_features`.
        Handles:
        ...
        :return: Entity as dict aligned with the `datasets.Dataset` schema.
        """
        ...
        # 2) If it’s not a model, we return as is
        return model
    ...
    def _convert_value_for_dataset(cls, value: Any, annotation: Any) -> Any:
        ...
        Parallel to `_annotation_to_values`.
        ...
        if origin in (list, tuple):
            ...
            if isinstance(value, (list, tuple)):  # noqa: UP038
                if len(args) == 1:
                    ...
                    # tuple[str, int, ...] => fallback to string
        ...
        if origin in (typing.Union, getattr(types, "UnionType", None)):
            ...
            if len(non_none_args) == 1:
                # It's Optional[T] or Union[T, None]
                return cls._convert_value_for_dataset(value, non_none_args[0])
            # For other Unions, fallback to string
            return str(value)
